...
class TestPlatformDefaults:
    ...
    def test_medium_tier_platforms(self):
        ...
        for plat in ("slack", "mattermost", "matrix", "whatsapp"):
            assert resolve_display_setting({}, plat, "tool_progress") == "new", plat

    def test_feishu_defaults_enable_streaming_and_full_progress(self):
        """Feishu gets OpenClaw-style defaults: streaming on + full tool progress."""
        from gateway.display_config import resolve_display_setting

        assert resolve_display_setting({}, "feishu", "tool_progress") == "all"
        assert resolve_display_setting({}, "feishu", "streaming") is True
