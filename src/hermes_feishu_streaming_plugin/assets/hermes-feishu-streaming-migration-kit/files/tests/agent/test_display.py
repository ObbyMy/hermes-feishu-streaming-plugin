...
class TestEditDiffPreview:
    ...
    def test_summarize_rendered_diff_sections_limits_file_count(self):
        ...
        assert "另外" in rendered[-1]


class TestCuteToolMessage:
    def test_browser_snapshot_uses_chinese_mode_label(self):
        result = get_cute_tool_message("browser_snapshot", {"full": True}, 1.2)
        assert "快照" in result
        assert "完整" in result

    def test_terminal_failure_suffix_is_localized(self):
        result = get_cute_tool_message("terminal", {"command": "false"}, 0.3, '{"exit_code": 1}')
